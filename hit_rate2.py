"""Summarize cue outcomes as a function of the animal's corridor position.
...
The script reports hit rates per position bin and produces visualizations of
both the hit-rate curve and the underlying cue/hit distributions when
matplotlib is available. A hit is defined as an event where a reward was
delivered (`hasGivenReward` set to True in the log).
"""
...
def plot_cue_distributions(
    aggregated: List[Tuple[int, int, int, float]],
    bin_size: int,
    output_path: pathlib.Path,
) -> None:
    """Save a bar chart comparing cue and hit counts per position bin."""

    if plt is None:
        raise RuntimeError("matplotlib is not available in this environment.")

    positions = [row[0] for row in aggregated]
    totals = [row[2] for row in aggregated]
    hits = [row[1] for row in aggregated]

    bar_width = max(bin_size * 0.4, 1)
    cue_positions = [pos - bar_width / 2 for pos in positions]
    hit_positions = [pos + bar_width / 2 for pos in positions]

    plt.figure(figsize=(10, 4))
    plt.bar(cue_positions, totals, width=bar_width, label="All cues", alpha=0.7)
    plt.bar(
        hit_positions,
        hits,
        width=bar_width,
    ...
    plt.xlabel("Original corridor position (bin start)")
    plt.ylabel("Cue count")
    plt.title("Cue and hit distributions vs. original corridor position")
...
def main() -> None:
    ...
    if not args.no_plot:
        ...
        distribution_plot_path = pathlib.Path(
            f"{output_prefix}_cue_distribution_by_position.png"
        )
        try:
            plot_cue_distributions(aggregated, args.bin_size, distribution_plot_path)
        except RuntimeError as exc:
            print(f"Skipping cue distribution plot: {exc}")
        else:
            print(
                "Wrote cue distribution plot to "
                f"{distribution_plot_path}"
            )
